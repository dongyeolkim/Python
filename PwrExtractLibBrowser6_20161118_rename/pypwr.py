...
#==============================================================================
        if self.gdsfilecheck.isChecked():
            self.strmout_tb.setText("StreamOut is not Checked\n")
        else:
...
#==============================================================================
        if self.hiercheck.isChecked():        
            ...
            extnetnames=self.extnetname.text().split(',')
            pwr=0;
            portion=30/len(extnetnames)
            for netname in extnetnames:
#              streamInKeys.streamin(self, netname)
              StreamIn.run(self, netname)
              self.update()
              self.progressBar.setValue(40+pwr*portion)
              ChangeCellName.runs(self,netname,pwr)
              pwr=pwr+1
        else:
            extnetnames=self.extnetname.text().split(',')
            pwr=0;
...
#      dialog.exec_()
       ...
       if dialog.exec_():
           ...
           cellName=""
           viewName=""
           libName=dialog.library.selectedItems()[0].text()
           cellName=dialog.cell.selectedItems()[0].text()
           viewName=dialog.view.selectedItems()[0].text()
           if libName != "" and cellName != "" and viewName != "" :
               self.libname.setText(libName) 
               self.cellname.setText(cellName) 
               self.viewname.setText(viewName) 
